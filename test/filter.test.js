...
const sample={id:"20261231-01",title:"송구영신 예배",startDate:"2025-12-31",endDate:"2026-01-02",departments:["juniors"],events:["worship"],place:"external"};
...
test("부서와 행사를 함께 필터링한다",()=>{ assert.equal(filterAlbums([sample],{department:"juniors",event:"worship",year:"2026",month:"",place:"external",keyword:"송구"}).length,1); });
...
test("장소로 필터링한다",()=>{ assert.equal(filterAlbums([sample],{department:"",event:"",year:"",month:"",place:"external",keyword:""}).length,1); assert.equal(filterAlbums([sample],{department:"",event:"",year:"",month:"",place:"second-temple",keyword:""}).length,0); });
